...
public class AdministratorBookkeeperRequestShowService implements AbstractShowService<Administrator, BookkeeperRequest> {
	...
	public void unbind(final Request<BookkeeperRequest> request, final BookkeeperRequest entity, final Model model) {
		...
		boolean pending = entity.getStatus().equals(BookkeeperRequestStatus.PENDING);
		boolean alreadyRejected = entity.getStatus().equals(BookkeeperRequestStatus.REJECTED);

		model.setAttribute("pending", pending);
		model.setAttribute("alreadyRejected", alreadyRejected);
