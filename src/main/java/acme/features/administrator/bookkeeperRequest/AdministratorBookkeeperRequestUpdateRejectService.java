...
@Service
public class AdministratorBookkeeperRequestUpdateRejectService implements AbstractUpdateService<Administrator, BookkeeperRequest> {
	...
	public boolean authorise(final Request<BookkeeperRequest> request) {
		...
		Boolean isNeitherAcceptedOrRejected = !request.getModel().getAttribute("status").equals(BookkeeperRequestStatus.ACCEPTED) || !request.getModel().getAttribute("status").equals(BookkeeperRequestStatus.REJECTED);

		return isNeitherAcceptedOrRejected;
	}
	...
	public void update(final Request<BookkeeperRequest> request, final BookkeeperRequest entity) {
		...
		entity.setStatus(BookkeeperRequestStatus.REJECTED);
